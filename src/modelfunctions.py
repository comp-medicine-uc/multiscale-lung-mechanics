...
def give_times_pressures_deltas_PCV(Nciclos, p_min,p_max):
    duration_step=3
    for ciclo in np.arange(1,Nciclos+1):
        if ciclo==Nciclos:    
            end=True
...
def give_times_fluxes_deltas_SUPER(Nciclos, vol_step,area): #solo inflado
    Tsyr=0.3 #inflation time
...
#        
        ...
        qs1=np.linspace(q1,q2,n1,endpoint=False)
        qs2=np.linspace(q2,q3,n2,endpoint=False)
        qs3=np.linspace(q3,q34,n23,endpoint=False)
        qs4=np.linspace(q34,q4,n3,endpoint=end)
        qsaux=np.concatenate((qs0,qs1,qs2,qs3,qs4))
...
def create_folders(lung,caso):
    ...
    dirNamefields = lung+'/'+'Results/Signals/'+caso
    try:
    ...
    # Create directory for 3D fields
    dirNamefields = lung+'/'+'Results/VTKfiles'
    try:
        os.mkdir(dirNamefields)
        print("Directory " , dirNamefields ,  " Created ") 
    except FileExistsError:
...
def give_tpqv(folder,name):
  ...
  ai=117.08
  at=ad+ai
  flujos=np.load('Right/Results/Signals'+folder+'/'+name+'fluxes.npy')*60+np.load('Left/Results/Signals'+folder+'/'+name+'fluxes.npy')*60
  presiones=(ad*np.load('Right/Results/Signals'+folder+'/'+name+'presionestodas.npy')+ai*np.load('Left/Results/Signals'+folder+'/'+name+'presionestodas.npy'))/at
  volumenes=np.load('Right/Results/Signals'+folder+'/'+name+'volumenes.npy')+np.load('Left/Results/Signals'+folder+'/'+name+'volumenes.npy')
...
def regression(folder,name):
    ...
    flujos=np.asarray((flujos))*1/60
    presiones=10.2*np.asarray((presiones))
...
def plotPCV():
    ...
    #ax1.fill_between(tiempos, presiones*10.2, y2=0,color='k',alpha=0.1)
    ax1.plot(tmenos25,pmenos25*10.2,'k')
    ax1.fill_between(tmenos25, y1=0, y2=pmenos25*10.2,color=color5,alpha=alpha5+alpha25,label='±$5\%$ ($f_0$)')
    ax1.fill_between(tmenos25, y1=0, y2=pmenos25*10.2,color=color25,alpha=alpha25,label='±$20\%$ ($f_0$)')
    ax1.fill_between(tmenos25, y1=0, y2=pmenos25*10.2,color='w')
    ...
    ax1.set_xlim(limxmin,limx)
    ax1.set_yticks([0, 5])
    ax1.set_yticklabels([r"$0$",r"$5$"], color="k", size=si)
    ax1.tick_params(bottom=False, labelbottom=False)
    ax1.set_ylim(0-0.05,5+0.5)
    labelx = -0.15  # axes coords
    ax1.yaxis.set_label_coords(labelx, 0.5)
    ax1.legend(bbox_to_anchor=(0,1.1,1,0.2),loc='upper left',mode='expand',ncol=6,shadow=True,prop=dict(size=si*0.9)) #bbox_to_anchor=(x0,y0, width, height)
    ...
    ax2.set_yticklabels([r"$-60$", r"$0$", r"$60$"], color="k", size=si)
    ax2.set_ylim(-70,70)
    ax2.tick_params(bottom=False, labelbottom=False)
    ...
    ax3.set_yticklabels([r"$0$", r"$0.5$"], color="k", size=si)
    labelx = -0.15  # axes coords
    ax3.yaxis.set_label_coords(labelx, 0.5)
    ax3.set_xticks([0, 1,2,3])
    ax3.set_xticklabels([r"$0$", r"$1$", r"$2$", r"$3$"], color="k", size=si)
...
def plotbaselinePCV():
    print('Sensitibity analysis for f_0')
...
def plotCompliancePCV():
    ...
    folder4='/fo0.72_mu10.33s00'
    folder5='/fo0.83_mu10.33s00'
    folder6='/fo0.59_mu10.33s00'
    folder7='/fo0.62_mu10.33s00'
    folder8='/fo0.76_mu10.33s00'
    folder9='/fo0.79_mu10.33s00'
    
    folders_compliance=[folder1,folder6,folder7,folder2,folder3,folder4,folder8, folder9,folder5]
    Cs=[]
    

    fos=[0.8*0.69,0.85*0.69,0.9*0.69,0.95*0.69 , 0.69, 0.69*1.05,0.69*1.1,0.69*1.15,0.69*1.2]
    for folder in folders_compliance:
        C=regression(folder,name)
        Cs.append(C)
    ...
    axs.set_xlabel(r'$f_0$ [-]',size=si)
    #axs.set_xticks([0.55, 0.6, 0.65,0.7,0.75,0.8])
    #axs.set_xticklabels([r"$0.55$", r"$0.60$", r"$0.65$", r"$0.70$", r"$0.75$", r"$0.80$"], color="k", size=si)
    
    axs.set_xticks([0.6, 0.7,0.8])
    axs.set_xticklabels([ r"$0.60$", r"$0.70$", r"$0.80$"], color="k", size=si)
    
    axs.set_yticks([80,90,100,110,120,130])
    axs.set_yticklabels([r"$80$", r"$90$", r"$100$", r"$110$", r"$120$", r"$130$"], color="k", size=si)
    #
    #
    ...
    folder4='/fo0.69_mu10.85s00'
    folder5='/fo0.69_mu12.4s00'
    folder6='/fo0.69_mu8.78s00'
    folder7='/fo0.69_mu9.3s00'
    folder8='/fo0.69_mu11.36s00'
    folder9='/fo0.69_mu11.88s00'
    

    folders_mus=[folder1,folder6,folder7,folder2,folder3,folder4,folder8, folder9,folder5]
    
    Cs=[]
    mus=[8.26,8.78,9.3,9.81,10.33,10.85,11.36,11.88,12.4]
    
    for folder in folders_mus:
        C=regression(folder,name)
        Cs.append(C)
    ...
    axs.set_ylabel(r'Compliance [ml/cm H$_2$O]', size=si)
    axs.set_xlabel(r'$\mu$ [kPa]',size=si)
    #axs.set_xticks([0, 100, 200])
    #axs.set_xticklabels([r"$0$", r"$100$", r"$200$"], color="k", size=si)
    
    axs.set_xticks([8,9,10,11,12,13])
    axs.set_xticklabels([r"$8$", r"$9$", r"$10$", r"$11$", r"$12$", r"$13$"], color="k", size=si)
    #
    #
    axs.axvline(x=10.33,color='k',linestyle='--')
    axs.set_yticks([80,90,100,110,120,130])
    axs.set_yticklabels([r"$80$", r"$90$", r"$100$", r"$110$", r"$120$", r"$130$"], color="k", size=si)
    #
    axs.set_xlim(0.8*10.33,1.2*10.33)
    axs.set_ylim(75,120)
...
def plotSUPER():
    def give_tpqv(folder,name):
      ...
      at=ad+ai
      tam=168
      flujos=(np.load('Right/Results/Signals'+folder+'/'+name+'fluxes.npy')[:tam]*60)+(np.load('Left/Results/Signals'+folder+'/'+name+'fluxes.npy')[:tam])*60
      presiones=(ad*np.load('Right/Results/Signals'+folder+'/'+name+'presionestodas.npy')[:tam]+ai*np.load('Left/Results/Signals'+folder+'/'+name+'presionestodas.npy')[:tam])/at
      volumenes=np.load('Right/Results/Signals'+folder+'/'+name+'volumenes.npy')[:tam]+np.load('Left/Results/Signals'+folder+'/'+name+'volumenes.npy')[:tam]
      volumenes=volumenes-volumenes[0]
      tiempos=np.load('Right/Results/Signals'+folder+'/'+name+'tiempos.npy')[:tam]
    ...
    for ii in np.arange(8):
        tt0=ii*duration
        #print(round(tt0,2))
        for jj in np.arange(len(tnormal)):
          if tt0==tnormal[jj]:
             ls.append(jj)
    ls=ls#+[len(tnormal)-1]  
    ...
    ax1.fill_between(pmenos5, y1=0, y2=0.00001,color=color5,alpha=alpha5+alpha25,label='±$5\%$ ($f_0$)')
    ax1.fill_between(pmenos5, y1=0, y2=0.00001,color=color25,alpha=alpha25,label='±$20\%$ ($f_0$)')
    ax1.fill_between(pmenos5, y1=0,  y2=0.1,color='w')
    
    ax1.set_xlabel(r'Aw. pressure [cm H$_2$O]',size=si)
    ax1.set_ylabel(r'Volume [L]',size=si)
    ax1.set_xlim(0,8)
    ax1.set_yticks([0, 0.2,0.4,0.6, 0.8])
    ax1.set_yticklabels([r"$0$", r"$0.2$",r"$0.4$",r"$0.6$", r"$0.8$"], color="k", size=si)
    ax1.set_xticks([0, 2,4,6,8])
    ax1.set_xticklabels([r"$0$", r"$2$", r"$4$", r"$6$",r"$8$"], color="k", size=si)
    ax1.set_ylim(0,0.69)
    #labelx = -0.15  # axes coords
    #ax1.yaxis.set_label_coords(labelx, 0.5)
    ax1.legend(bbox_to_anchor=(0,0.94,1,0.2),loc='upper left',mode='expand',ncol=6,shadow=True,prop=dict(size=si*0.9)) #bbox_to_anchor=(x0,y0, width, height)
    
    
    
    ax1.fill_betweenx( np.asarray(vmenos25[ls])[0:im], x1=10.2*np.asarray(pmenos25[ls])[0:im], x2=10.2*np.asarray(pmas25[ls])[0:im]-0.02,color=color25,alpha=alpha25)
    ax1.fill_betweenx( np.asarray(vmenos5[ls])[0:im], x1=10.2*np.asarray(pmenos5[ls])[0:im], x2=10.2*np.asarray(pmas5[ls])[0:im],color=color5,alpha=alpha5)
    
    ax1.plot(10.2*pmas5[ls],vmas5[ls],color=color5,linestyle=style5,alpha=alpha5)
    ax1.plot(10.2*pmenos5[ls],vmenos5[ls],color=color5,linestyle=style5,alpha=alpha5)
    
    ax1.plot(10.2*pnormal[ls],vnormal[ls],color=colornormal,linestyle=stylenormal,alpha=alphanormal)
    ...
    si=15
    #auxiliares para que leged sean en orden 2 y 20, con colores y orden correctos
    ax1.fill_between(pmenos5, y1=0, y2=0.00001,color=color5,alpha=alpha5+alpha25,label='±$5\%$ ($\mu$)')
    ax1.fill_between(pmenos5, y1=0, y2=0.00001,color=color25,alpha=alpha25,label='±$20\%$ ($\mu$)')
    ax1.fill_between(pmenos5, y1=0,  y2=0.1,color='w')
    ...
    ax1.fill_betweenx( np.asarray(vmenos25[ls])[0:im], x1=10.2*np.asarray(pmenos25[ls])[0:im], x2=10.2*np.asarray(pmas25[ls])[0:im],color=color25,alpha=alpha25)
    ax1.fill_betweenx( np.asarray(vmenos5[ls])[0:im], x1=10.2*np.asarray(pmenos5[ls])[0:im], x2=10.2*np.asarray(pmas5[ls])[0:im],color=color5,alpha=alpha5)
...
def plotbaselineSUPER():
    ...
    si=15
    #auxiliares para que leged sean en orden 2 y 20, con colores y orden correctos
    ax1.plot(10.2*pnormal[ls],vnormal[ls],color=colornormal,linestyle=stylenormal,alpha=alphanormal,label='$      f_0$=0.69  &  $\mu$=10.33')
    ax1.set_xlabel(r'Aw. pressure [cm H$_2$O]',size=si)
    ax1.set_ylabel(r'Volume [L]',size=si)
    ax1.set_xlim(0,8)
    ax1.set_yticks([0, 0.2,0.4,0.6, 0.8])
    ...
    plt.tight_layout()     
    plt.show()
    plt.savefig('baseline_super.pdf')
